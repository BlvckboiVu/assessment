...
def add_another_expense():
    ...
    if not date.get() or not payee.get() or not desc.get() or not amnt.get() or not MoP.get():
        ...
        connector.execute('INSERT INTO ExpenseTracker (Date, Payee, Description, Amount, ModeofPayment) VALUES (?, ?, ?, ?, ?)', (date.get_date(), payee.get(), MoP.get()))
        connector.commit()
...
root = Tk()
root.title('PytonGeeks Expense Tracker')
...
buttons_frame = Frame(root, bg=dataentery_frame_bg)
...
ddl = OptionMenu(data_entry_frame, MoP, *['Cash', 'Cheque', 'Credit Card', 'Debit Card', 'Paytm', 'Google Pay', 'Razorpay'])
ddl.place(x=160, y=305)
ddl.configure(width=10, font=entry_font)

Button(data_entry_frame, text='Add expense', command=add_another_expense, font=btn_font, width=30, bg=hlb_btn_bg).place(x=10, y=450)
...
table.heading('Description', text='Desciption', anchor=CENTER)
